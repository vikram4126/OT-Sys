...
function VulnRow({ vuln, onRefresh, isMitigated, onNavigate }) {
  ...
  const TYPE = (()=>{ const rt=vuln.relevanceType||vuln.relevance_type||'Direct';
    const map={Direct:{l:'Asset CVE',c:'#0F766E',b:'#E7F7F1'},Inferred:{l:'Inferred',c:'#B54708',b:'#FEF0DA'},Systemic:{l:'Systemic',c:'#510DBC',b:'#F1EAFE'}};
  ...
  return (
    <>
      <div style={{
        display:'grid', gridTemplateColumns:'22px 1fr 110px 90px 70px 92px 60px',
        gap:10, padding:'10px 14px', alignItems:'center',
        background:'transparent', borderBottom:`1px solid ${C.border}`,
        opacity: isDimmed ? 0.6 : 1, transition:'opacity .2s',
      }}>
        {/* Status icon */}
        <div style={{display:'flex',alignItems:'center',justifyContent:'center'}}>
          {isMitigated ? <MitigatedIcon/> : isAccepted ? <AcceptedIcon/> : <div style={{width:18}}/>}
        </div>

        {/* Title + short description — click opens the risk breakdown (brain) */}
        <div style={{minWidth:0, cursor:'pointer'}} onClick={()=>setShowExplain(true)} title="See how the risk score was calculated">
          <div style={{display:'flex',alignItems:'center',gap:7,marginBottom:1}}>
            <span style={{fontFamily:'monospace',fontSize:11,fontWeight:600,color:'#64748B'}}>{vuln.vuln_id}</span>
            {isFlagged && <span title="AI flagged low confidence — actioned in Workspace" style={{ fontSize:9, fontWeight:600, color:C.muted, background:'#F1F5F9', border:`1px solid ${C.border}`, padding:'1px 6px', borderRadius:3 }}>Review</span>}
            {vuln.risk_score_overridden && <span title="A metric was overridden by a consultant" style={{ fontSize:9, fontWeight:700, color:C.violet, background:'#F1EAFE', padding:'1px 6px', borderRadius:3 }}>overridden</span>}
          </div>
          <div style={{fontSize:13,fontWeight:500,color:isDimmed?C.muted:C.text,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',textDecoration:isDimmed?'line-through':'none'}}>
            {vuln.title}
          </div>
          {shortDesc && <div style={{fontSize:11,color:C.muted,marginTop:1,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',opacity:.8}}>{shortDesc}</div>}
        </div>

        {/* Vuln type */}
        <span><span title={TYPE.l} style={{fontSize:10,fontWeight:700,color:TYPE.c,background:TYPE.b,padding:'2px 8px',borderRadius:20,whiteSpace:'nowrap'}}>{TYPE.l}</span></span>

        {/* Exploitable */}
        {(()=>{ const ex=vulnExploitability(vuln); const exC=ex.level==='High'?'#B42318':ex.level==='Medium'?'#B54708':'#067647';
          return <span title={ex.reason} style={{textAlign:'center',cursor:'help'}}>
            <span style={{fontSize:10.5,fontWeight:700,color:exC,background:`${exC}14`,padding:'2px 9px',borderRadius:20,whiteSpace:'nowrap'}}>{ex.level}</span>
          </span>;
        })()}

        {/* Risk score — click opens the calculation breakdown (brain modal) */}
        {(()=>{ const rs=typeof vuln.risk_score==='number'?vuln.risk_score:null; const c=rs==null?C.muted:rs>=8.5?'#B42318':rs>=6.5?'#C2410C':rs>=4?'#B54708':'#067647';
          return (<span style={{textAlign:'center'}}>
            <button onClick={()=>setShowExplain(true)} title="How was this score calculated?" style={{background:'none',border:'none',cursor:'pointer',padding:0,fontFamily:'inherit',display:'inline-flex',flexDirection:'column',alignItems:'center'}}>
              <span style={{fontSize:15,fontWeight:700,color:c}}>{rs==null?'—':rs.toFixed(1)}</span>
              {typeof vuln.confidence==='number' && <span style={{display:'block',fontSize:8.5,color:C.muted}}>conf {vuln.confidence}%</span>}
            </button>
          </span>);
        })()}

        {/* Status — click to change (status only) */}
        <div style={{position:'relative'}}>
          <button onClick={()=>setStatusOpen(o=>!o)} style={{padding:'3px 8px',borderRadius:4,fontSize:11,fontWeight:600,color:sb.color,background:sb.bg,border:'none',cursor:'pointer',fontFamily:'inherit',whiteSpace:'nowrap',display:'flex',alignItems:'center',gap:4}}>{sb.label} ▾</button>
          {statusOpen && (
            <>
              <div onClick={()=>setStatusOpen(false)} style={{position:'fixed',inset:0,zIndex:40}}/>
              <div style={{position:'absolute',top:'100%',left:0,marginTop:4,zIndex:41,background:'#fff',border:`1px solid ${C.border}`,borderRadius:8,boxShadow:'0 8px 24px rgba(0,51,141,.15)',minWidth:150,overflow:'hidden'}}>
                {[['Open','Open'],['Closed','Closed'],['Accepted Risk','Closed · risk accepted']].map(([val,lbl])=>(
                  <button key={val} onClick={()=>setStatus(val)} style={{display:'block',width:'100%',textAlign:'left',padding:'8px 12px',fontSize:12,color:C.text,background:(vuln.status||'Open')===val?'#F5F8FD':'#fff',border:'none',cursor:'pointer',fontFamily:'inherit'}}>{lbl}</button>
                ))}
                {isMitigated && <div style={{padding:'7px 12px',fontSize:10.5,color:C.muted,borderTop:`1px solid ${C.border}`}}>Auto-closed: a linked mitigation was actioned.</div>}
              </div>
            </>
          )}
        </div>

        {/* Actions: brain (explain) + 3-dots (open full overview, view/edit inside) */}
        <div style={{display:'flex',gap:4,justifyContent:'flex-end'}}>
          <button onClick={()=>setShowExplain(true)} title="Explain (AI confidence & rubric)" style={{width:28,height:28,borderRadius:6,background:`${C.violet}0E`,border:`1px solid ${C.violet}25`,cursor:'pointer',color:C.violet,display:'flex',alignItems:'center',justifyContent:'center'}}><Brain/></button>
          <button onClick={()=>setShowDetail(true)} title="Open vulnerability — view or edit" style={{width:28,height:28,borderRadius:6,background:`${C.navy}08`,border:`1px solid ${C.navy}22`,cursor:'pointer',color:C.navy,display:'flex',alignItems:'center',justifyContent:'center'}}>
...
// ── Main ──────────────────────────────────────────────────────────────────────
export default function VulnerabilitiesTab({ onNavigate = () => {} }) {
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',gap:20}}>
      <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',flexWrap:'wrap',gap:12}}>
        <div>
          <h2 style={{margin:0,fontSize:20,fontWeight:700,color:C.text,letterSpacing:-.3}}>Vulnerabilities</h2>
          <div style={{display:'flex',alignItems:'center',gap:8,marginTop:4,flexWrap:'wrap'}}>
            <span style={{fontSize:13,color:C.muted}}>{vulns.length} findings · {counts.open} open</span>
            {(()=>{
              const flagged = withMitigation.filter(isFlaggedFn);
              if (!flagged.length) return null;
              return (
                <span style={{display:'inline-flex',alignItems:'center',gap:5,padding:'2px 9px',borderRadius:20,
                  background:'#F1F5F9',border:`1px solid ${C.border}`,fontSize:11,fontWeight:600,color:C.muted}}>
                  <span style={{width:6,height:6,borderRadius:'50%',background:C.muted}}/>
                  {flagged.length} flagged for review — actioned in Workspace
                </span>
              );
            })()}
            {complementary.length>0 && (
              <button onClick={()=>setShowComplementary(true)} style={{display:'inline-flex',alignItems:'center',gap:5,padding:'2px 9px',borderRadius:20,
                background:'#EEF2FA',border:`1px solid ${C.border}`,fontSize:11,fontWeight:600,color:C.navy,cursor:'pointer',fontFamily:'inherit'}}>
                <span style={{width:6,height:6,borderRadius:'50%',background:C.navy}}/>
                {complementary.length} additional CVE{complementary.length===1?'':'s'} found via CPE lookup — review
              </button>
            )}
          </div>
        </div>
        <Btn onClick={()=>setShowAdd(true)}>+ Add Finding</Btn>
      </div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:12}}>
        {[
          {label:'Open', value:counts.open, color:'#B54708'},
          {label:'Critical (open)', value:counts.critical, color:'#B42318'},
          {label:'Closed', value:counts.closed, color:'#067647'},
          {label:'Risk accepted', value:counts.accepted, color:'#510DBC'},
        ].map(({label,value,color})=>(
          <div key={label} style={{background:'#fff',borderRadius:10,padding:'14px 18px',border:`1px solid ${C.border}`}}>
            <div style={{fontSize:26,fontWeight:600,color,letterSpacing:-.5,marginBottom:6}}>{value}</div>
      ...
      <Card style={{padding:'11px 14px'}}>
        <div style={{display:'flex',gap:10,flexWrap:'wrap',alignItems:'center'}}>
          <div style={{flex:1,minWidth:180}}>
            <Input value={search} onChange={e=>{setSearch(e.target.value);setPage(1);}} placeholder="Search by ID, title, asset, CVE…"/>
          </div>
          <div style={{display:'flex',gap:5,alignItems:'center'}}>
            <span style={{fontSize:12,color:C.muted}}>Severity:</span>
            {['All','Critical','High','Medium','Low'].map(o=><Pill key={o} label={o} active={crit===o} onClick={()=>{setCrit(o);setPage(1);}}/>)}
          </div>
          <div style={{display:'flex',gap:5,alignItems:'center'}}>
            <span style={{fontSize:12,color:C.muted}}>Status:</span>
            {['All','Open','Closed','Accepted'].map(o=><Pill key={o} label={o} active={status===o} onClick={()=>{setStatus(o);setPage(1);}}/>)}
          </div>
          <div style={{display:'flex',gap:5,alignItems:'center'}}>
            <span style={{fontSize:12,color:C.muted}}>Zone:</span>
            <div style={{width:150}}><Select value={zoneF} onChange={e=>{setZoneF(e.target.value);setPage(1);}} options={[{value:'All',label:'All zones'},...zones.map(z=>({value:z.id,label:z.name}))]}/></div>
          </div>
          {/* Review sort toggle */}
      ...
      <Card style={{padding:0,overflow:'hidden'}}>
        <div style={{display:'grid',gridTemplateColumns:'22px 1fr 110px 90px 70px 92px 60px',gap:10,padding:'9px 14px',background:'#F5F8FD',borderBottom:`1px solid ${C.border}`}}>
          {['','Finding','Type','Exploitable','Risk','Status',''].map((h,i)=>(
            <div key={i} style={{fontSize:10,fontWeight:600,color:C.muted,textTransform:'uppercase',letterSpacing:.8}}>{h}</div>
          ))}
        </div>
        {paged.length===0
